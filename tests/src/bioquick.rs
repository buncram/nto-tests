...
pub unsafe fn bio_bypass() -> ! {
    ...
    */
    ...
    ifram_src.fill(0);
    ifram_dst.fill(0);
    basic_u32(&mut bio_ss, &mut main_mem_src, &mut main_mem_dst, 0, "Main->main", concurrent);

    main_mem_src.fill(0);
    main_mem_dst.fill(0);
    basic_u32(&mut bio_ss, ifram_src, &mut main_mem_dst, 0x40, "ifram0->main", concurrent);

    ifram_src.fill(0);
    main_mem_dst.fill(0);
    basic_u32(&mut bio_ss, &mut main_mem_src, ifram_dst, 0x80, "Main->ifram1", concurrent);

    main_mem_src.fill(0);
    ifram_dst.fill(0);
    basic_u32(&mut bio_ss, ifram_src, ifram_dst, 0xC0, "ifram0->ifram1", concurrent);
