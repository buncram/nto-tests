...
// This constant sums up all the active tests in this suite.
const BIO_TESTS: usize =
    1  // get_id
    + 21 // dma (4*5 + 1)
    + 1  // stack test
    + 3  // hello word, hello multiverse, aclk_tests
    + 1  // fifo_basic
    + 1  // host_fifo_tests
    + 1  // spi_test
    + 1  // i2c_test
...
// Conditionally add 1 to the total test count if the optional "bio-mul" feature is enabled.
#[cfg(not(feature = "bio-mul"))]
const BIO_TESTS_FINAL: usize = BIO_TESTS;
#[cfg(feature = "bio-mul")]
const BIO_TESTS_FINAL: usize = BIO_TESTS + 1;

crate::impl_test!(BioTestsSam, "BIO_SAM", BIO_TESTS_FINAL);

impl TestRunner for BioTestsSam {
    fn run(&mut self) {
        let id = get_id();
        ...
        self.passing_tests += bio_tests::dma::dma_basic(false, 3); // 4 tests
        self.passing_tests += bio_tests::dma::dma_basic(true, 3);  // 4 tests
        self.passing_tests += bio_tests::dma::dma_bytes();         // 4 tests
        self.passing_tests += bio_tests::dma::dma_u16();           // 4 tests
        self.passing_tests += bio_tests::dma::dma_coincident(3);   // 4 tests
        self.passing_tests += bio_tests::dma::dma_multicore(3);    // 1 test

        self.passing_tests += bio_tests::units::hello_world();
        self.passing_tests += bio_tests::arith::stack_test();

        // This test only runs if the target hardware supports multiplication and the
        // "bio-mul" feature is enabled during compilation.
        #[cfg(feature = "bio-mul")]
        {
            // safety: this is safe only if the target supports multiplication
            self.passing_tests += unsafe { bio_tests::arith::mac_test() }; // 1 test
        }
